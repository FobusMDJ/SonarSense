...
import { AlertTriangle, ChevronLeft, ChevronRight, Loader2, MapPin, ScanSearch } from 'lucide-react'
import {
  ApiError, getDetections, getModelStats, getVaeStats, vaeFrameImageUrl, VAE_PANEL_FILES,
  type ModelOutputStats, type RawDetection, type VaeStats, type LogSummary,
} from '../lib/api'
            ...
            return (
              <g key={det.id}>
                ...
                <text x={x} y={Math.max(12, y - 6)} fill={color} fontSize={Math.max(14, naturalSize.w / 32)} fontWeight={700}>
                  {displayClassification(det.class_name)} {Math.round(det.confidence_score)}%
                </text>
              </g>
            )
          })}
...
function ConfidenceBreakdown({ detection }: { detection: RawDetection }) {
  ...
  return (
    <div className="ds-confidence-card">
      <header>
        <strong>{displayClassification(detection.class_name)}</strong>
...
function GeolocationCard({ detection }: { detection: RawDetection }) {
  ...
  return (
    <div className="ds-geolocation-card">
      <header><MapPin size={17} /><strong>{displayClassification(detection.class_name)}</strong></header>
      {located ? (
        <dl>
...
export function PipelineView({ logs, selectedLogId, onSelectLog }: PipelineViewProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([getDetections(selectedLogId), getModelStats(selectedLogId), getVaeStats(selectedLogId)])
      .then(([dets, stats, vae]) => {
        if (cancelled) return
        setDetections(dets.filter(d => !isHumanClass(d.class_name)))
        setModelStats(stats); setVaeStats(vae); setFrameIndex(0); setStep(0)
      })
      ...
      {!loading && !error && (
        <>
          ...
          <section className="ds-panel ds-model-panel">
            <header className="ds-panel-title"><h2>YOLO26 detection statistics</h2></header>
            {modelStats && modelStats.n_detections > 0 ? (
              <div className="ds-model-metrics">
                {Object.entries(modelStats.counts_by_class).map(([cls, count]) => (
                  <article key={cls}>
                    <div><span>{cls}</span></div>
                    <strong>{count}</strong>
                    <small>mean conf {modelStats.mean_confidence_by_class[cls]?.toFixed(1)}%</small>
                  </article>
                ))}
              </div>
